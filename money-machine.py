...
kucoin_client = kc.Client(API_KEY, API_SECRET, 'Shell.hydr4')
...
async def send_periodic_message(client, channel_username, message):
    ...
    while True:
        ...
        await asyncio.sleep(18000)  # Wait for 30 seconds before sending the next message
...
async def send_successful_trade_message(client, channel_username, currency_name, trade_counter):
    ...
    current_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    message = f'[{current_time}] Successfully pumped. Trade number: [{trade_counter}], the coin is: [{currency_name}].'
...
async def listen_to_channel(channel_username):
    ...
    async def new_message(event):
        ...
        if currency_name:
            ...
            # Send a message when a trade is successful
